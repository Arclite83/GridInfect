...
| 7 | **Audio.** One click per hop, pitched up per ray depth, and a synth solve voice (swept, driven sawtooth) after the winning wave's last click. Synthesised at runtime. | `python3 tools/render_audio.py` renders the same formulas to WAV to listen to off-device; the levels (`ClickVolume`, `ChimeVolume` in `HopClickAudio`) are set from those renders, not from a phone. |
