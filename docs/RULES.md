...
- **The last piece.** Because the win check runs first, a placement whose
  arm ends on a trap or a switch behaves differently depending on *when* it
  is played: early it resets the board or repels infection back off it, last
  it simply wins. So a board with a switch or a trap can have a placement
  that is only ever legal as the final one — its **last piece**. 84 of the
  128 Legacy levels have one (`docs/last_piece_classic.json`, generated by
  `tools/gen_last_piece_golden.py`; nothing generated has one, since no
  generated board carries a switch or a trap). A player can play one early
  and lift it again; the Lock tool cannot, so it passes over any placement
  whose arms reach a switch or a trap until it is the one that wins
  (ARCHITECTURE §3, `piece.lock`).
