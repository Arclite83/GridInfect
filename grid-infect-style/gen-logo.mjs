...
if (MAIN) {
  const files = {
    ...
    "monogram_bg":       monogramBackground(),
  };
  ...
  if (PNG) {
    ...
    const chromium = pw.chromium ?? pw.default.chromium;   // ESM or the CJS index.js of a global install
    ...
    const jobs = [
      ...
      ...[1024, 432].map(px => ["monogram_bg", px / 1024, true]),
    ];
