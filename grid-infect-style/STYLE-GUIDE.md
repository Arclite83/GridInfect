...
## 7. HUD
Height 56px, items bottom-aligned: the two chips and the level label. Level label Chakra Petch 26px in ink, 0.06em tracking. A second row at 52px from the top carries the Share Tech Mono 12px caption (the mode readout: `LEGACY`, `DAILY`, `ENDLESS T3`) left-aligned under MENU and the counter badge right-aligned under RESET.
