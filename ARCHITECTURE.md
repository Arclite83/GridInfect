...
|---|---|---|
| `Bloodhound.Engine` | Action dispatch/registry/log, `MiniJson`, `Pcg32`, `Work` (background jobs by priority, a main-thread pump for completions, `Map` to fan a job across the cores). Game-agnostic — the piece that moves to the next game (the logic game) unchanged. | nothing (`noEngineReferences`) |
