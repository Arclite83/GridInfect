...
| `Profile` | solved Legacy ids and solved world indices {id → set} — the whole of progression — plus best times ms[5], run counts[5], muted, skin, language tag, daily bests {date → ms}, daily streak and last date, endless best streak[5], lock wallet (start 5, free grants capped at 10), the tutorial record (the first-open offer answered; the highest step beaten plus one), and the two retired gate fields nothing reads | pure data; serialization only via `SaveCodec` (versioned JSON, expand/contract; v2 added `worlds`, v3 the daily/endless fields, v4 `locks`, v5 the solved sets, v6 `skin`, v7 `tutorialSeen`/`tutorialStep` — a v4 save derives them from its gates, since solving N is what opened N+1; v8 `lang`) |
