...
|---|---|---|
| `Bloodhound.Engine` | Action dispatch/registry/log, `MiniJson`, `Pcg32`, `Work` (background jobs by priority, a main-thread pump for completions, `Map` to fan a job across the cores — on its own lowest-priority threads, never the pool's, as wide as the adapter says at the time). Game-agnostic — the piece that moves to the next game (the logic game) unchanged. | nothing (`noEngineReferences`) |
...
| `Profile` | solved Legacy ids and solved world indices {id → set} — the whole of progression — plus best times ms[5], run counts[5], muted, skin, language tag, daily bests {date → ms}, daily streak, last date and best streak ever, endless best streak[5], lock wallet (start 5, free grants capped at 10), the tutorial record (the first-open offer answered; the highest step beaten plus one), and the two retired gate fields nothing reads | pure data; serialization only via `SaveCodec` (versioned JSON, expand/contract; v2 added `worlds`, v3 the daily/endless fields, v4 `locks`, v5 the solved sets, v6 `skin`, v7 `tutorialSeen`/`tutorialStep` — a v4 save derives them from its gates, since solving N is what opened N+1; v8 `lang`, v9 `dailyStreakBest`) |
...
| `GameState` | mode (Classic / FreePlay / World / Daily / Endless / Tutorial) + classic id / free-play run / world id and index / daily run / endless run / tutorial step + `Session` + `Profile` + the level's stored `Solution` (vector for Legacy, generator's otherwise) | wall-clock time enters **only** through action inputs |
...
| `daily.complete` | `nowMs` | DailyActions | solved: elapsed, personal best per date; the streak counts dates solved on the day, so it moves only when the run's date is the clock's own UTC date (`StreakGrantDue` on a rung of the ladder in `Rewards`: the first 3 ever, then every 7th); rejects a backward clock |
...
| `tutorial.seen` | — | TutorialActions | the first-open offer has been answered, taken or skipped; idempotent |
